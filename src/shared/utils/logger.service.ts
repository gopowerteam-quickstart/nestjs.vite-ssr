...
export interface LoggerMapperItem {
  name: string;
  styles: {
    type: string;
    date: string;
...
export class LoggerService {
  ...
  private static systemInfo: Record<string, string>;
   ...
   * @param message
   */
  public static printLoggor(
    level: keyof typeof LoggerMapper,
    message: string[],
  ) {
  ...
  public static storageLoggor(
    ...
    // 获取日志Storage
    const storage = JSON.parse(localStorage.getItem(storgetKey) || '') || [];
    // 更新日志Storage
    storage.push(Object.assign(record, { message }));
    localStorage.setItem(storgetKey, storage);
  }
   ...
   * @returns
   */
  private static formatLogger(logger: LoggerMapperItem) {
    // 获取日志函数
